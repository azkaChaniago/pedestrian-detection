...
class CurvedLaneDetection:
    ...
    def sliding_window(self,img, nwindows=9, margin=150, minpix = 1, draw_windows=True):
        ...
        # else:
        #     left_lane_inds = np.zeros(right_lane_inds)

        left_lane_inds = np.concatenate(left_lane_inds)
        right_lane_inds = np.concatenate(right_lane_inds)
        # print('left_lane_inds: {}; right_lane_inds: {}'.format(left_lane_inds, right_lane_inds))
        ...
        # print('leftx: {}, lefty: {}; rightx:{}, righty:{}'.format(leftx, lefty, rightx, righty))
    ...
    def draw_lanes(self, img, left_fit, right_fit, dst=np.float32([(0.43,0.65),(0.58,0.65),(0.1,1),(1,1)])):
        ploty = np.linspace(0, img.shape[0]-1, img.shape[0])
        ...
        inv_perspective = self.inv_perspective_warp(color_img, dst_size=(width,height), dst=dst)
